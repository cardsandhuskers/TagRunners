...
public class SetHunterSelectionCommand implements CommandExecutor {
    private final Tag plugin;

    public SetHunterSelectionCommand(Tag plugin) {
        this.plugin = plugin;
    }
        ...
        if(sender instanceof Player p) {
            if (p.isOp()) {
                if (args.length == 2) {
                    ...
                    p.sendMessage(ChatColor.RED + "Incorrect Usage: /setTagKitSpawn [ArenaNumber] [TeamSideNumber 1 or 2]");
                }
            } else {
