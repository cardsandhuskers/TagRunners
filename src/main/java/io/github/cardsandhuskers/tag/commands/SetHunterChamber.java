            ...
            if (p.isOp()) {
                if (args.length == 2) {
                    try {
                        ...
                        plugin.getConfig().set("ArenaHunterChambers.Arena" + arenaIndex + "." + teamSide, location);
                        plugin.saveConfig();

                        p.sendMessage("Arena" + arenaIndex + " Team " + teamSide + " Hunter Chamber Location set at:\nWorld: " + location.getWorld() + "\nX: " + location.getX() + " Y: " + location.getY() + " Z: " + location.getZ());
